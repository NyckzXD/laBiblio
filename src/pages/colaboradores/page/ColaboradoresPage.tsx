...
export function ColaboradoresPage() {
  ...
  const filtered = useMemo(
    () =>
      listColaboradores?.filter((c) =>
        c.nome.toLowerCase().includes(search.toLowerCase()),
      ) ?? [],
    [listColaboradores, search],
  );

  const recentEntries =
    listColaboradores?.filter(
      (s) => Date.now() - s._creationTime < 30 * 24 * 60 * 60 * 1000,
    ).length ?? 0;
  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in">
      ...
      />
      <Separator />
      <ListColaboradores listColaboradores={filtered} />
    </div>
  );
}